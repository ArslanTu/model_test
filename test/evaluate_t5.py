...
def eval(args, subject, model, tokenizer, dev_df, test_df):
    ...
    with open(os.path.join("results", f"results_{args.model}.csv"), mode='a') as file:
        file.write(f"{subject},{acc}\n")
    return cors, acc, all_probs


def main(args):
    if args.model == "flan-t5-3b":
        model_name = "google/flan-t5-xl"
    elif args.model == "t5-3b":
        model_name = "t5-3b"
    model = T5ForConditionalGeneration.from_pretrained(model_name, load_in_8bit=False)
    ...
    if os.path.exists(os.path.join("results", f"results_{args.model}.csv")):
        os.remove(os.path.join("results", f"results_{args.model}.csv"))
    with open(os.path.join("results", f"results_{args.model}.csv"), mode='a') as file:
